...
async function syncWechatPosts(isUpdate) {
  ...
  while (isContinue) {
    ...
    console.info("posts item length:" + posts.item.length)
    ...
    for (var index in posts.item) {
      ...
      console.error("22222 existPost:" + existPost);
      ...
      if (!existPost.data.length) {
        ...
        //不需要更新直接继续
        if (!isUpdate) {
          console.error("00000 no need update");
          continue
        }
...
async function getWechatPosts(accessToken, offset, count) {
  ...
  let rbody = (typeof result === 'object') ? result : JSON.parse(result);
  console.error("rbody.getWechatPosts" + rbody.getWechatPosts)
  return rbody;
}
